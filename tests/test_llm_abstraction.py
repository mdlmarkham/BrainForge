...
from src.services.llm.health_monitor import (
    HealthMonitor,
    HealthStatus,
    get_health_monitor,
)
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_half_open(self):
        ...
        # Simulate timeout by calling can_execute which should check the timeout
        # and transition to HALF_OPEN if timeout has passed
        # We need to mock time.time() to simulate the timeout
        with patch('time.time') as mock_time:
            # Set time to be after the reset timeout
            mock_time.return_value = cb.last_failure_time + cb.reset_timeout + 1
