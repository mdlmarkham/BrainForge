...
class IntegrationProposalService(SQLAlchemyService[IntegrationProposal]):
    ...
    async def get_proposal_by_content_source(self, db: AsyncSession, content_source_id: UUID) -> Optional[IntegrationProposal]:
        """Get integration proposal by content source ID."""
        
        from sqlalchemy import select
        from ..models.orm.integration_proposal import IntegrationProposal as IntegrationProposalORM
