...
class HealthCheckResult(BrainForgeBaseModel, TimestampMixin):
    ...
    response_time: float | None = Field(None, description="Response time in milliseconds")
    error_message: str | None = Field(None, description="Error message if check failed")
    error: str | None = Field(None, description="Error message (alias for error_message)")
...
class HealthMetrics(BrainForgeBaseModel, TimestampMixin):
    ...
    last_check_time: float | None = Field(None, description="Timestamp of last check")
...
class ProviderAvailabilityManager:
    ...
    async def get_best_provider(self, preference_order: list[str] | None = None) -> str | None:
