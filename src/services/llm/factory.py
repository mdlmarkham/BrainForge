...
from src.config.llm import LLMProviderConfig, OllamaConfig, OpenAIConfig
...
class ProviderInstance(BrainForgeBaseModel, TimestampMixin):
    ...
    circuit_breaker: Optional[CircuitBreaker] = Field(None, description="Circuit breaker instance")
    error_handler: Optional[ErrorHandler] = Field(None, description="Error handler instance")
...
class LLMProviderFactory:
    ...
    async def create_provider(self, config: LLMProviderConfig,
                            provider_name: Optional[str] = None) -> ProviderInstance:
    ...
    async def get_provider(self, provider_name: str) -> Optional[ProviderInstance]:
