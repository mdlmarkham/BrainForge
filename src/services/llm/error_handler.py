...
class ErrorHandler:
    ...
    def __init__(self, circuit_breaker: CircuitBreaker, retry_strategy: Optional[RetryStrategy] = None):
    ...
    async def handle_operation(self, func: Callable[..., T], max_retries: Optional[int] = None) -> T:
...
class FallbackStrategy(BrainForgeBaseModel):
    ...
    async def get_next_fallback(self, current_provider: str, attempted_providers: list[str]) -> Optional[str]:
