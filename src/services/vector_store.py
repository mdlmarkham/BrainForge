...
class VectorStore:
    ...
    async def store_vector(self, note_id: str, vector: List[float], 
                          model_name: str, model_version: str) -> Optional[Embedding]:
        ...
        try:
            ...
            if not self._validate_vector(vector):
                logger.error(f"Invalid vector provided for storage")
                return None
    ...
    async def update_vector(self, note_id: str, new_vector: List[float],
        ...
        try:
            if not self._validate_vector(new_vector):
                logger.error(f"Invalid vector provided for update")
                return None
    ...
    async def get_vector(self, note_id: str) -> Optional[List[float]]:
